...
const generateWord = (asset, transactions, baseFileName, depreciationView, issuesTransfers) => {
  ...
  if (issuesTransfers && issuesTransfers.length > 0) {
    issuesTransfers.filter(issue => issue.propertyNumber === asset.propertyNumber).forEach((issue) => {
      tableData.push([
        new TableCell({ children: [new Paragraph(issue.date || '')], width: { size: convertMillimetersToTwip(20) } }),
        new TableCell({ children: [new Paragraph(issue.parNumber || '')], width: { size: convertMillimetersToTwip(30) } }),
        new TableCell({ children: [new Paragraph('')], width: { size: convertMillimetersToTwip(50) } }),
        new TableCell({ children: [new Paragraph('')], width: { size: convertMillimetersToTwip(35) } }),
        new TableCell({ children: [new Paragraph('')], width: { size: convertMillimetersToTwip(35) } }),
        new TableCell({ children: [new Paragraph('')], width: { size: convertMillimetersToTwip(35) } }),
        new TableCell({ children: [new Paragraph(issue.accountableUser || '')], width: { size: convertMillimetersToTwip(35) } }),
        new TableCell({ children: [new Paragraph('')], width: { size: convertMillimetersToTwip(30) } }),
        new TableCell({ children: [new Paragraph('')], width: { size: convertMillimetersToTwip(21) } }),
        new TableCell({ children: [new Paragraph('')], width: { size: convertMillimetersToTwip(21) } })
      ]);
    });
  }
  ...
  if (transactions && transactions.length > 0) {
    transactions.forEach((transaction) => {
      tableData.push([
        ...
        new TableCell({ children: [new Paragraph(transaction.poNumber || '')], width: { size: convertMillimetersToTwip(30) } }),
